...
class LegacyCheckpointCallback(pl.Callback if pl is not None else object):
    ...
    def _checkpoint_payload(
        ...
        *,
        ...
        return {
            "step": step,
            "model": str(pl_module.hparams.model_name),
            ...
            "context_length": int(pl_module.hparams.context_length),
            "batch_size": int(pl_module.hparams.batch_size),
            "kl_weight": float(pl_module.hparams.kl_weight),
            "reverse_kl_weight": float(pl_module.hparams.reverse_kl_weight),
            "ce_weight": float(pl_module.hparams.ce_weight),
            "aux_letter_loss_weight": float(pl_module.hparams.aux_letter_loss_weight),
            "aux_letter_enable_thinking": bool(pl_module.hparams.aux_letter_enable_thinking),
            "latent_dropout": float(compactor_cfg.get("latent_dropout", 0.0)),
            "loss_mode": str(pl_module.hparams.loss_mode),
            "target_mode": str(pl_module.hparams.target_mode),
            "score_mode": str(pl_module.hparams.score_mode),
            "eval_max_new_tokens": int(getattr(pl_module.hparams, "eval_max_new_tokens", 192)),
            "init_checkpoint": str(getattr(pl_module.hparams, "init_checkpoint", "") or ""),
            "use_chat_template": bool(pl_module.hparams.use_chat_template),
            "enable_thinking": bool(pl_module.hparams.enable_thinking),
            "eval_enable_thinking": bool(pl_module.hparams.eval_enable_thinking),
            "balanced_answer_sampling": bool(pl_module.hparams.balanced_answer_sampling),
            "trainable_scope": str(pl_module.hparams.trainable_scope),
